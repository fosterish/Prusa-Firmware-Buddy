    ...
    void get_slot_info(Printer::Params &params) {
        ...
        for (size_t i = 0; i < params.slots.size(); i++) {
            if (params.slot_mask & (1 << i)) {
                ...
                params.slots[i].hardened = config_store().get_nozzle_is_hardened(nozzle);
                params.slots[i].high_flow = config_store().get_nozzle_is_high_flow(nozzle);
            }
        }
    }
} // namespace
...
void MarlinPrinter::set_slot_info(size_t idx, const SlotInfo &info) {
    ...
    config_store().set_nozzle_is_hardened(idx, info.hardened);
    config_store().set_nozzle_is_high_flow(idx, info.high_flow);
}
